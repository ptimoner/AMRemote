...
timeFolder=$(echo 20$(date +"%y%m%d%H%M")
mkdir -p $1/out/$timeFolder/slum_reports
# mkdir -p slurm_reports
sbatch -W -o $1/out/$timeFolder/slum_reports/regions.out script_hpc_get_regions.sh $1 $2 $3
REGIONS=$(cat $1/inputs.json | jq -r '.index | join(",")')
# echo $REGIONS
# REGIONS=$(echo 2)

# echo 'Submitting main analyses (job array)...'
sbatch -a $REGIONS -o ./slurm_reports/%a_%A.out script_hpc.sh $1 $2 $timeFolder
